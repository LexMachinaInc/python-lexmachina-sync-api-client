...
class QueryDistrictCase:
    ...
    def query_one_page(self, query, options=None):
        response = self.district_case_query.post(path="/query-district-cases", data=query)
        if response:
            return response.get("caseIds")
        return []

    def query_all_pages(self, query, options=None):
        query_results = query.execute()
        cases = []
        query.set_page_size(10)
        while True:
            page_cases = self.query_one_page(query_results, options)
    ...
    def query_district_case(self, query, options):
        ...
        if options and options['pageThrough']:
            return self.query_all_pages(query, options)
        else:
            return self.query_one_page(query_results, options)
