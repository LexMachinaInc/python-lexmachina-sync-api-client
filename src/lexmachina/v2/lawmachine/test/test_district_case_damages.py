# coding: utf-8

"""
    Lex Machina API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 20241105
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from lawmachine.models.district_case_damages import DistrictCaseDamages

class TestDistrictCaseDamages(unittest.TestCase):
    """DistrictCaseDamages unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> DistrictCaseDamages:
        """Test DistrictCaseDamages
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `DistrictCaseDamages`
        """
        model = DistrictCaseDamages()
        if include_optional:
            return DistrictCaseDamages(
                name = '',
                type = '',
                judgment_source = '',
                awarded_to_party_ids = [
                    56
                    ],
                awarded_against_party_ids = [
                    56
                    ],
                amount = None,
                awarded = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(),
                negated = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(),
                reinstated = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date()
            )
        else:
            return DistrictCaseDamages(
                name = '',
                type = '',
                judgment_source = '',
                awarded_to_party_ids = [
                    56
                    ],
                awarded_against_party_ids = [
                    56
                    ],
                amount = None,
                awarded = datetime.datetime.strptime('1975-12-30', '%Y-%m-%d').date(),
        )
        """

    def testDistrictCaseDamages(self):
        """Test DistrictCaseDamages"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
